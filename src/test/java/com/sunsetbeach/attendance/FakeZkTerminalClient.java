...
public class FakeZkTerminalClient implements ZkTerminalClient {
    ...
    /**
     * Queues one poll's outcome for this device - a {@code List<RawAttendancePunch>} to return
     * (windowedReadUnsupported left {@code null}, i.e. "this poll didn't test it" - true for most
     * tests, which aren't about that flag at all), a {@link FakeOutcome} to also control it
     * explicitly, or a {@code RuntimeException} to throw.
     */
    ...
    /** Lets a test say what this one poll found out about windowed-read support - see TerminalPollResult#windowedReadUnsupported's own javadoc. */
    public record FakeOutcome(List<RawAttendancePunch> punches, Boolean windowedReadUnsupported) {
    }
    ...
    public TerminalPollResult poll(AttendanceDeviceEntity device, LocalDateTime since, Integer knownRecordSize) {
        ...
        Integer recordSize = since == null ? DEFAULT_RECORD_SIZE : knownRecordSize;
        if (queue == null || queue.isEmpty()) {
            return new TerminalPollResult(List.of(), recordSize, null);
        }
        Object outcome = queue.poll();
        if (outcome instanceof RuntimeException e) {
            throw e;
        }
        if (outcome instanceof FakeOutcome fakeOutcome) {
            return new TerminalPollResult(fakeOutcome.punches(), recordSize, fakeOutcome.windowedReadUnsupported());
        }
        ...
        return new TerminalPollResult(punches, recordSize, null);
    }
}
